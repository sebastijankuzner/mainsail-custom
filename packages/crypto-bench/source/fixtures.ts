...
const NUMBER_OF_VALIDATORS = 51;

export const validators: SecretKey[] = [];
export const validatorPublicKeys: Buffer[] = [];
for (let i = 0; i < NUMBER_OF_VALIDATORS; i++) {
    const secretKey = SecretKey.fromKeygen(Buffer.alloc(32, i + 1));
    validators.push(secretKey);
    validatorPublicKeys.push(Buffer.from(secretKey.toPublicKey().toBytes()));
}

export const validatorSignatures: Buffer[] = [];
for (const validator of validators) {
    validatorSignatures.push(Buffer.from(validator.sign(randomBytes(32)).toBytes()));
}
